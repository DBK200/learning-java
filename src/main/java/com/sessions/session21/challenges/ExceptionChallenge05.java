...
//        String s = "src/main/java/com/sessions/session21/challenges/Messages_RO.txt";
//        s = "Messages_RO.txt";
//        System.out.println(Arrays.toString(getAppMessages(s)));
//        s = null;
...
//    }
//
//    private static String[] getAppMessages(String path){
//        // This will automatically close the resource at the end
//        try(BufferedReader reader = new BufferedReader(new java.io.FileReader(path))) {
//            String line;
//            StringBuilder sbResult = new StringBuilder();
...
//    private static boolean calculator(Scanner sc) throws RuntimeException {
//
//        // List of operators
//        final List<Character> OPERATORS = List.of('+', '-', '*', '/', '^', 'r');
//
...
//    }
//
//    private static TreeMap<String, BigDecimal> calculate(Double operand1, Double operand2, char operator)
//            throws RuntimeException {
//
//        // Division by 0 validation
//        if (operand2 != null
...
//    }
//
//    private static BigDecimal power(Double base, Double exponent){
//
//        boolean isExponentNegative = BigDecimal.valueOf(exponent).signum() == -1;
//
//        if (BigDecimal.valueOf(base).signum() == 0 && isExponentNegative)
...
//    }
//
//    private static void formatOutput(String[] arValues, List<Character> operators) throws RuntimeException{
//
//        Double dOperand1 = Double.parseDouble(arValues[0]),
//               dOperand2 = (arValues.length >= 3) ? Double.parseDouble(arValues[2]) : null;
//
...
//        char chOperator = arValues[1].toLowerCase().charAt(0);
//        if (!operators.contains(chOperator))
//            throw new RuntimeException(addInMessage(chOperator, messages[11], -2));
//        else {
//            if (arValues.length == 2 && chOperator != 'r')
//                throw new RuntimeException(addInMessage(chOperator, messages[12], -3));
//        }
//
//        Map.Entry<String, BigDecimal> entryResult = calculate(dOperand1, dOperand2, chOperator).firstEntry();
//
...
//    }
//
//    private static void formatOutput(RuntimeException e){
//
//        String sResponse = String.format("%s: %s%n%n",
//                e.getClass().getSimpleName(), e.getMessage());
//
...
//    }
//
//    private static void printTable(String message) {
//        System.out.printf("┌%1$s┐%n│ %2$s │%n└%1$s┘%n%n",
//                "─".repeat(message.trim().length() + 2), message.trim());
//    }
//
//    private static String[] getAppMessages(Scanner sc, String language) throws FileNotFoundException {
//
//        // Gets the messages file
//        sc = new Scanner(new File(String.format("src/main/java/com/sessions/session21/challenges/Messages_%s.txt",
//                language.toUpperCase())));
//
//        StringBuilder sbResult = new StringBuilder();
//        boolean bAppend = false;
...
//            if (sTemp != null){
//                if (bAppend) sbResult.append(sTemp).append("\n");
//                if (!sTemp.isEmpty()
//                        && sTemp.length() >= 3
//                        && sTemp.substring(0, 3).equalsIgnoreCase("<p>"))
//                    bAppend = true;
//                if (!sTemp.isEmpty()
//                        && sTemp.length() >= 4
//                        && sTemp.substring(0, 4).equalsIgnoreCase("</p>"))
//                    bAppend = false;
//            }
//        }
//
...
//    }
//
//    private static <T> String addInMessage(T value, String message, int position) throws RuntimeException{
//
//        // Message validation
//        if (message == null || message.isEmpty())
...
//        // Insertion point validation
//        if (Math.abs(position) > message.length())
//            throw new RuntimeException(messages[13]);
//
//        StringBuilder sbResult = new StringBuilder(message);
//
//        // A negative position number means that the position
